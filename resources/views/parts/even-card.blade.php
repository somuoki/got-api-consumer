        ...
        <div class="flex items-start justify-between w-full">
            <div class="pl-3 w-full">
                <p tabindex="0" class="focus:outline-none text-xl font-medium leading-5 text-gray-800">
                    <a href="book/{{ $specific['id'] }}">
                        {{ $specific['name'] }}
                    </a>
                </p>
                <p tabindex="0" class="focus:outline-none text-sm leading-normal pt-2 text-gray-500">
                    @if(isset($specific['comment_count'])){{ $specific['comment_count'] }} Comments @endif
                    @if(isset($specific['founded'])) Founded In {{ $specific['founded'] }} To {{ $specific['diedOut'] }} @endif
                    @if(isset($specific['gender']))Gender {{ $specific['gender'] }} Age {{ $specific['age'] }} @endif
                </p>
            </div>
...
{{--            Number Of pages : {{ $book['numberOfPages'] }}, Publishers : {{ $book['publisher'] }},--}}
{{--            Country : {{ $book['country'] }}, Media Type : {{ $book['mediaType'] }}, Release Date : {{ $book['released'] }}--}}
        </p>
