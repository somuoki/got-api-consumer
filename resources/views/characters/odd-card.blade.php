    ...
    <div tabindex="0" aria-label="card 1" class="focus:outline-none lg:w-4/12 lg:mr-7 lg:mb-0 mb-7 bg-white p-6 shadow rounded">
        ...
        <div class="flex items-center border-b border-gray-200 pb-6">
            ...
            <div class="flex items-start justify-between w-full">
                <div class="pl-3 w-full">
                    ...
                    <p tabindex="0" class="focus:outline-none text-sm leading-normal pt-2 text-gray-500">Gender : {{ $character['gender'] }}, Age :{{ $character['age'] }}</p>
                </div>
        ...
        <div class="px-2">
            <p tabindex="0" class="focus:outline-none text-sm leading-5 py-4 text-gray-600">
                @unless($character['culture'] == '') Culture : {{ $character['culture'] }}, @endunless @unless($character['born'] == '')Born : {{ $character['born'] }},@endunless
                @unless($character['died'] == '')Died : {{ $character['died'] }},@endunless @unless(empty($character['titles']) )Titles : @foreach($character['titles'] as $titles) {{ $titles }}, @endforeach,@endunless
                @unless(empty($character['playedBy']))Played By : @foreach($character['playedBy'] as $playedBy) {{ $playedBy }},  @endforeach @endunless
            </p>
